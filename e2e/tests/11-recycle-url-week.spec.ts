import {
  ...
  createTestWeek, cleanupTestWeek,
  futureFridayLabel, toDashLabel,
} from '../fixtures';

// 测试周期动态取下一周五：仅未来周可被普通管理员删除（进回收站），写死的周期会随时间过期
...
test.describe('11 回收站周期的 URL 跳转', () => {
  ...
  test('URL 指向有效周期：正常跳转选中', async ({ page }) => {
    ...
    await page.goto(`/?weekLabel=${WEEK}&dept=${encodeURIComponent('综合管理部')}`);
    await expect(page.locator('.ant-card-head-title', { hasText: '本周工作内容' })).toBeVisible({ timeout: 15000 });
    await expect(page.locator('.ant-select-selection-item').first()).toHaveText(WEEK_DASH);
  });

  test('URL 指向回收站周期：不得选中、无报错', async ({ page }) => {
    // 删除测试周期（进回收站）
    const req = await request.newContext();
    await req.delete(`${(await import('../fixtures')).API}/reports/${WEEK}`, {
    ...
    expect(selected).not.toBe(WEEK_DASH);
