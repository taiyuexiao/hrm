...
- **全库数据体检（2026-09-14 傍晚）**：新增 `scripts/audit_weekly_reports.py`（8 类可疑形态只读审计）+ 修复脚本扩展第三/四阶段（空文本任务清理、文本字段 JSON → 格式化文本、空数组置空）。首轮扫出 101 项（G 类文本字段原始 JSON 98 项、C 类空文本任务 3 项），全部修复后复检 **0 问题**。执行前备份 `hr.db.bak2-20260914`
